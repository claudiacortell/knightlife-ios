//
//  BlockDetailModuleMasthead.swift
//  Glancer
//
...
class BlockDetailModuleMasthead: TableModule
{
	let controller: BlockDetailTableViewController
	
	init(_ controller: BlockDetailTableViewController)
	{
...
//		mainSection.addCell(TableCell("masthead.id", callback:
//		{ viewCell, cell in
//			(cell as! BlockTableTextViewCell).textBox = self.controller.block.blockId.displayName
//		}))
...
//		}
		...
		let buttonSection = TableSection()
		buttonSection.addCell(TableCell("popup"))
		container.addSection(buttonSection)
	}
}
